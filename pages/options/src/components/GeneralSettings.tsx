    ...
    <section className="space-y-6">
      <div
        ...
        <div className="space-y-4">
          ...
          {/* napi: guide mode toggle */}
          <div className="flex items-center justify-between">
            <div>
              <h3 className={`text-base font-medium ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>Guide mode</h3>
              <p className={`text-sm font-normal ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>
                Show one step at a time and let you perform it yourself, instead of the agent acting on its own.
              </p>
            </div>
            <div className="relative inline-flex cursor-pointer items-center">
              <input
                ...
                onChange={e => updateSetting('guideMode', e.target.checked)}
                className="peer sr-only"
              />
              <label
                htmlFor="guideMode"
                className={`peer h-6 w-11 rounded-full ${isDarkMode ? 'bg-slate-600' : 'bg-gray-200'} after:absolute after:left-[2px] after:top-[2px] after:size-5 after:rounded-full after:border after:border-gray-300 after:bg-white after:transition-all after:content-[''] peer-checked:bg-yellow-600 peer-checked:after:translate-x-full peer-checked:after:border-white peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-yellow-300`}>
                <span className="sr-only">Guide mode</span>
              </label>
            </div>
          </div>
